 ...
 *
 * 기본 = advisory(exit 0, 보고만). `--enforce` = dead-link 또는 orphan 있으면 exit 1.
 * `--strict-orphans` 없이 --enforce 면 dead-link 만 차단(orphan 은 경고). 7/3 데드라인 비차단 기본.
 *
 * 설계 doc: docs/progress/99.dev-comm/2026-06-15-structure-harness-design.md §4.
 * 한계(정직): registry 표 컬럼 기반 ID 참조만 검사. 내용 정합·유닛테스트 @spec 태그 커버리지는 미검사(코드 게이트 몫).
 */
import fs from "fs";
import path from "path";

const ROOT = process.env.TK_PROJECT_ROOT || process.cwd();
...
console.log("V모델 추적성 검사 — docs/progress/0N.*/INDEX.md");
